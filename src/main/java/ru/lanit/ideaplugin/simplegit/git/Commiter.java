...
public class Commiter extends DialogWrapper implements CheckinProjectPanel, TypeSafeDataProvider {
    ...
    @NotNull private final List<Commiter.CommitExecutorAction> myExecutorActions;
    ...
    @NotNull private final Commiter.MyChangeProcessor myDiffDetails;
    ...
    @Nullable private final Commiter.CommitAction myCommitAction;
    ...
    public static boolean commitChanges(@NotNull Project project,
        ...
        if (executor == null) {
            return commitChanges(project, changes, initialSelection, collectExecutors(project, changes), true, comment, customResultHandler);
        }
        else {
            return commitChanges(project, changes, initialSelection, singletonList(executor), false, comment, customResultHandler);
        }
    }
...
//                        message("commit.dialog.no.changes.detected.title"));
        ...
        Commiter dialog =
                new Commiter(project, changes, initialSelection, executors, showVcsCommit, manager.getDefaultChangeList(),
                        manager.getChangeListsCopy(), singleVcs, false, comment, customResultHandler);
    ...
    public static void commitAlienChanges(@NotNull Project project,
        ...
        new Commiter(project, changes, null, emptyList(), true, AlienLocalChangeList.DEFAULT_ALIEN, singletonList(changeList),
                vcs, true, comment, null).show();
    }

    private Commiter(@NotNull Project project,
                     @NotNull List<Change> changes,
                     @Nullable LocalChangeList initialSelection,
                     @NotNull List<CommitExecutor> executors,
                     boolean showVcsCommit,
        ...
        myDiffDetails = new Commiter.MyChangeProcessor(myProject);
        ...
        myIsAlien = isAlien;
        if (isAlien) {
            myCommitMessageArea.setChangeLists(newArrayList(changeLists));
            ...
            MultipleChangeListBrowser browser =
                    new MultipleChangeListBrowser(project, changeLists, (List)changes, initialSelection, () -> updateWarning(),
                            () -> myHandlers.forEach(CheckinHandler::includedChangesChanged), myShowVcsCommit);
        ...
        myBrowser.setDiffBottomComponent(new Commiter.DiffCommitMessageEditor(myProject, myCommitMessageArea));
        ...
        myCommitAction = myShowVcsCommit ? new Commiter.CommitAction(getCommitActionName()) : null;
        myExecutorActions = map(executors, Commiter.CommitExecutorAction::new);
        ...
        if (!myCommitOptions.isEmpty()) {
            mainPanel = new JPanel(new Commiter.MyOptionsLayout(mySplitter, myCommitOptions, JBUI.scale(150), JBUI.scale(400)));
    ...
    private void executeDefaultCommitSession(@Nullable CommitExecutor executor) {
        ...
        ensureDataIsActual(() -> {
            try {
                Commiter.DefaultListCleaner defaultListCleaner = new Commiter.DefaultListCleaner();
    ...
    public Set<AbstractVcs> getAffectedVcses() {
        return myShowVcsCommit ? myBrowser.getAffectedVcses() : emptySet();
    }
    ...
    public Collection<VirtualFile> getRoots() {
        ...
        return map2SetNotNull(myBrowser.getCurrentDisplayedChanges(), change -> vcsManager.getVcsRootFor(ChangesUtil.getFilePath(change)));
    }
    ...
    public boolean vcsIsAffected(String name) {
        return ProjectLevelVcsManager.getInstance(myProject).checkVcsIsActive(name) &&
                exists(myBrowser.getAffectedVcses(), vcs -> Comparing.equal(vcs.getName(), name));
    }
    ...
    private class MyChangeProcessor extends ChangeViewDiffRequestProcessor {
        ...
        protected void selectChange(@NotNull Change change) {
            //noinspection unchecked
            myBrowser.select((List)singletonList(change));
        }
